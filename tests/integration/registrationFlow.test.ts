...
const mockApiHandler = {
  ...
  decklistUpdate: async (updateData: any) => {
    const { registrationId, decklist, bringingDecklistOnsite, existingRegistration } = updateData
    ...
    // Determine status
    const hasDecklist = updateFields.decklist !== undefined ? 
      updateFields.decklist !== null : 
      existingRegistration.decklist !== null
    const bringingOnsite = updateFields.bringingDecklistOnsite !== undefined ? 
...
describe('Integration Tests - Full Registration Flow', () => {
  ...
  describe('Complete Registration Workflow', () => {
    ...
    it('should complete full workflow for event with decklist requirement', async () => {
      ...
      expect(decklistResult.registration.decklist).toBe('4 Pikachu\n4 Thunder Shock\n4 Pokeball')
    })
  ...
  describe('Re-registration After Cancellation', () => {
    it('should allow complete re-registration workflow after cancellation', async () => {
      ...
      expect(reregistrationResult.registration.bringingDecklistOnsite).toBe(false) // Reset
      ...
      expect(finalResult.registration.decklist).toBe('4 Mewtwo\n4 Psychic Energy\n4 Ultra Ball')
    })
    ...
    it('should prevent registration if user has active (non-cancelled) registration', async () => {
      ...
      await expect(mockApiHandler.register(eventData)).rejects.toThrow('Already registered')
    })
  })
  ...
  describe('Event Capacity Management', () => {
    ...
    it('should reject registration when event is at capacity', async () => {
      ...
      await expect(mockApiHandler.register(eventData)).rejects.toThrow('Event is full')
    })
  })
})